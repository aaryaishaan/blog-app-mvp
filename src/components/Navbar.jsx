...
const Navbar = () => {
  ...
  return (
    <>
      {/* make the wrapper fixed at the top like your original */}
      <ResizableNavbar className="fixed top-0 left-0 w-full z-20">
        {/* Desktop / large screens */}
        <NavBody>
          {/* LEFT: logo (keeps react-router Link so it doesn't reload) */}
          ...
          {/* CENTER: nav links (uses Link to avoid full page reloads)
              NavBody in the library is already centered / absolute, so these will sit visually in the middle.
          */}
          ...
          <div className="flex items-center gap-3">
            ...
            {/* Note: NavBody from library is hidden on small screens (lg:flex). For the toggle we rely on MobileNav header */}
          </div>
        </NavBody>
          ...
          <MobileNavMenu isOpen={isMobileMenuOpen} onClose={() => setIsMobileMenuOpen(false)}>
            <div className="flex w-full flex-col gap-4">
              ...
              <div className="flex w-full flex-col gap-3 mt-2">
                <Link to="/login" onClick={() => setIsMobileMenuOpen(false)}>
                  <button className="w-full px-4 py-2 rounded-lg text-white bg-blue-700">
                    Login/Signup
                  </button>
                </Link>
      ...
      {/* Spacer so page content doesn't sit under fixed navbar. Adjust h-16 if your navbar height changes. */}
      <div className="h-16" />
    </>
  );
};
